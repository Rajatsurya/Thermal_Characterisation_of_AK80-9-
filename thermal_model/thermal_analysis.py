...
if __name__ == "__main__":
    csv_file_path = 'test2_thermal_data.csv'  # CSV with time and current
    csv_file_path2 = 'interpolated_300Hz_test_2.csv'
    ...
    thermal_cam_degree_c_col = 'Temp_C'
    ...
    # plt.plot(time_t, cases, label='Housing temperature predicted by the thermal model', color='blue')
    plt.plot(time_t,case_temp_ItoT, label = 'Housing temperature predicted by the defaut ItoT model', color='green')
    plt.plot(time_t,thermal_cam_degree_c, label = 'Housing temperature as measured from the thermal camera', color='red')
    # plt.plot(time_t,)
    plt.xlabel('Time (s)')
    plt.ylabel('Temperature (°C)')
    plt.title('Motor Houing Thermal plots')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
